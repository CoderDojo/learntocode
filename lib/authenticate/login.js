...
function authenticateUser(authenticate, response) {
  ...
  pool.getConnection(function(err, connection) {
    connection.query("SELECT users.name, users.session_hash, users.email_hash FROM users WHERE email=? and password=?",[authenticate.email, authenticate.password], function(err, result) {
      if(err) {
