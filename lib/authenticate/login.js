...
function authenticateUser(authenticate, response) {
  ...
  pool.getConnection(function(err, connection) {
    connection.query("SELECT users.name, users.session_hash, users.email_hash FROM users WHERE email=? and password=?  and enabled=1",[authenticate.email, authenticate.password], function(err, result) {
      if(err) {
         ...
         error.send('Error retrieving session info','',response);
          ...
          if(result.length > 0) {            
            ...
            connection.release();
            error.send('Error logging in',['Invalid username or password or your account is locked'],response);
          }
      }
    });
 });
