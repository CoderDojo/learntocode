...
function authenticateUser(authenticate, response) {
  ...
  pool.getConnection(function(err, connection) {
    connection.query("SELECT name, email_hash, session_hash FROM users WHERE email_hash=? and session_hash=? and enabled=1",[authenticate.email, authenticate.session_hash], function(err, result) {
      if(err) {
