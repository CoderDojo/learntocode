...
function insertCss(request, response, userid) {
  ...
  var json = jsonCss(request.body.css, response)
  try {
    json = json.replace(/(\n|\r|\r\n)$/, '')
    ...
    error.send('Error in your CSS code, please fix it!','',response);
  }
}
...
function jsonCss(css, response) {
  ...
  try {
    ...
    error.send('Error in your CSS, please fix it!','',response);
  }
}