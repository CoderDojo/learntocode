...
function retrieveHtml(request, response) {
      ...
      pool.getConnection(function(err, connection) {
        connection.query("SELECT user.name, user.coderdojo, user.city, user.publish, html.html_uuid, html.html, css.json as css FROM html html, css css, users user WHERE user.id=css.userid and user.publish=true and user.id=html.userid " +
             " UNION " +
             " SELECT user.name, user.coderdojo, user.city, user.publish, html.html_uuid, html.html, '' as css FROM html html, users user WHERE user.publish=true and user.id=html.userid AND user.id NOT IN (select userid from css)", function(err, result) {
        if(err) {
