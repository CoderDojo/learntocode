...
function retrieveHtml(request, response) {
      ...
      pool.getConnection(function(err, connection) {
        connection.query("SELECT user.name, user.coderdojo, user.city, html.html_uuid, html.html, css.json as css FROM html html, css css, users user WHERE user.id=css.userid and user.id=html.userid", function(err, result) {
        if(err) {
